...
class Messages extends StatefulWidget {
  int pageIndex;
  Messages({Key? key, required this.pageIndex}) : super(key: key);
...
class _MessagesState extends State<Messages> with SingleTickerProviderStateMixin{
  ...
  Future refresh() async {
    ...
    for(int i = 0; i< 8; i++){
      if(postsList[i].isEmpty){
        postsList[i] = await HttpService.GET(HttpService.API_SEARCH_LIST, HttpService.paramsSearch(page: page, perPage: 5, topic: topics.values.toList()[i]));
      }
      else{
        postsList[i].addAll(await HttpService.GET(HttpService.API_SEARCH_LIST, HttpService.paramsSearch(page: page, perPage: 5, topic: topics.values.toList()[i])));
      }
    }
...
Widget list(){
  ...
  return ListTile(
    leading: ClipRRect(
      ...
      //           )
      //       ),
      //   errorWidget: (context, url, error) => const Image(image: AssetImage('assets/cache/img_1.png')),
      // ),
    ),
